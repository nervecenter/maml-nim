...
const ESCAPE_MAP: Table[char, string] = {
    '"':    """"""",
    '\\':   """\\""",
    '/':    """/""",
    'b':    """\b""",
...
export proc parse(source: string): any {
    ...
    proc parseKeyword<T>(name: string, value: T) {
        if (ch !== name[0]) return
    ...
    proc isWhitespace(ch: string) {
        return ch === ' ' || ch === '\n' || ch === '\t' || ch === '\r'
    }

    proc isHexDigit(ch: string) {
        return (ch >= '0' && ch <= '9') || (ch >= 'A' && ch <= 'F')
    }

    proc isDigit(ch: string) {
        return ch >= '0' && ch <= '9'
    }

    proc isKeyChar(ch: string) {
        return (
            (ch >= 'A' && ch <= 'Z') ||
            (ch >= 'a' && ch <= 'z') ||
            (ch >= '0' && ch <= '9') ||
    ...
    proc expectValue(value: unknown) {
        if (value === undefined) {
            throw new SyntaxError(errorSnippet())
        }
    }
